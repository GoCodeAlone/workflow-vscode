...
> This extension requires workflow engine v0.80.13 or later.
...
The extension connects to `workflow-lsp-server` (downloaded automatically on first activation) and surfaces diagnostics as you type in any `workflow.yaml`, `workflow.yml`, `app.yaml`, `app.yml`, `infra.yaml`, or `infra.yml` file:
...
Bundled JSON Schemas provide baseline validation even when the LSP server is disabled: `schemas/workflow-config.schema.json` validates `workflow.yaml`, `workflow.yml`, `app.yaml`, `app.yml`, `infra.yaml`, and `infra.yml`; `schemas/wfctl-manifest.schema.json` validates `wfctl.yaml` and `wfctl.yml`.
...
```sh
go install github.com/GoCodeAlone/workflow/cmd/wfctl@v0.80.13
```
...
```sh
go install github.com/GoCodeAlone/workflow/cmd/workflow-lsp-server@v0.80.13
```
